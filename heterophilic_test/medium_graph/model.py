    ...
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, 
        ...
        if not self.pre_linear:
            if gnn=='GAT':
                ...
                self.local_convs.append(GATv2Conv(in_channels, hidden_channels))
            elif gnn=='Transf':
                self.local_convs.append(TransformerConv(in_channels,hidden_channels, heads=heads, concat=False, beta=True))
            elif gnn=='DGATv2':
                self.local_convs.append(DGATv2Conv(in_channels,hidden_channels, heads=heads,concat=False))
            elif gnn=='DTrans':
                 self.local_convs.append(DiffAttConv(in_channels,hidden_channels, heads=heads,concat=False, beta=True))
            else:
        ...
        for _ in range(local_layers):
            if gnn=='GAT':
                ...
                self.local_convs.append(GATv2Conv(hidden_channels, hidden_channels))
            elif gnn=='Transf':
                     self.local_convs.append(TransformerConv(hidden_channels,hidden_channels, heads=heads, concat=False, beta=True))
            elif gnn=='DGATv2':
                     self.local_convs.append(DGATv2Conv(hidden_channels,hidden_channels, heads=heads,concat=False))
            elif gnn=='DTrans':
                     self.local_convs.append(DiffAttConv(hidden_channels,hidden_channels, heads=heads,concat=False,beta=True))
            else:
